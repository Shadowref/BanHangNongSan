...
// ======== PHÂN TRANG CHỈ DÀNH CHO FILE NÀY ========
$limit = 6;
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
if($page < 1) $page = 1;
$offset = ($page - 1) * $limit;

// ====== Lấy tổng sản phẩm (chỉ sản phẩm hiện) ======
$total_result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM sanpham WHERE trangthai='hien'");
$total_row = mysqli_fetch_assoc($total_result);
$total_items = $total_row['total'];
$total_pages = ceil($total_items / $limit);

// ====== Lấy dữ liệu sản phẩm giá cao + số lượng đánh giá (chỉ hiện) ======
$sql = "
SELECT sp.*, l.tenloai, 
...
if($total_pages > 1){
    for($i=1;$i<=$total_pages;$i++){
        if($i == $page){
            ...
            echo "<a href='?content=giacao&page=$i' style='margin:0 5px; padding:5px 10px; background:#f1f1f1; color:#333; border-radius:5px; text-decoration:none; transition:0.3s;'>$i</a>";
        }
    }
}
echo '</div>';
