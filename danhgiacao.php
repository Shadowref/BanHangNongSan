...
// Tạo điều kiện WHERE theo trạng thái
$where = "WHERE sp.trangthai = 'hien'"; // mặc định chỉ hiện sp có trạng thái "hien"

if ($status == 'conhang') {
    $where .= " AND sp.soluong > 0";
...
// Đếm tổng số sản phẩm theo trạng thái
$total_result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM sanpham sp $where");
...
.product-card {border:1px solid #ddd;border-radius:8px;padding:10px;text-align:center;background:#fff;
    box-shadow:0 2px 6px rgba(0,0,0,0.1);transition:transform 0.3s,box-shadow 0.3s;width:100%;}
...
.product-card .status {font-size:13px;font-weight:bold;margin-bottom:5px;}
.product-card form input[type="number"] {width:50px;margin-bottom:5px;}
.product-card form button {background:#27ae60;color:white;padding:5px 10px;border:none;border-radius:4px;
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $soluong_ban = intval($row['soluong_ban']);
    ...
    // Trạng thái sản phẩm
    echo '<div class="status" style="color:'.($soluong_con>0?'green':'red').'">';
    echo ($soluong_con>0) ? 'Còn hàng' : 'Hết hàng';
    echo '</div>';
    ...
    // Số lượng bán / còn
    echo '<div class="sold">';
    echo 'Đã bán: '.$soluong_ban.' | Còn: '.$soluong_con;
    echo '</div>';

    // Form thêm giỏ hàng (chặn hết hàng)
    if($soluong_con > 0){
        ...
        echo '<button style="background:#ccc;cursor:not-allowed;">⛔ Hết hàng</button>';
    }
...
if($total_pages > 1){
    for($i=1;$i<=$total_pages;$i++){
        if($i == $page){
            ...
            echo "<a href='index.php?content=danhgiacao&page=$i' 
                     style='margin:0 5px; padding:5px 10px; background:#f1f1f1; color:#333; border-radius:5px; text-decoration:none; transition:0.3s;'>$i</a>";
        }
    }
}
echo '</div>';
